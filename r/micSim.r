...
micSimParallel <- function(initPop=NULL, immigrPop=NULL, initPopList = c(), immigrPopList = c(),
                           ...
                           fixInitStates = c(), maxAge=99, simHorizon, fertTr=c(), monthSchoolEnrol=c(), cores=1, seeds=1254, transitionFuns){
  ...
  if (itt<=nL) {
    ...
    popIt <- micSim(initPop=initPopL, immigrPop=immigrPopL, transitionMatrix=transitionMatrix, 
                      ...
                      monthSchoolEnrol=monthSchoolEnrol, transitionFuns=transitionFuns)
